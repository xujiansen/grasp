...
public class StatusBarUtil {
    ...
    public static void setTranslucentForCoordinatorLayoutWithColor(Activity activity, int color) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        transparentStatusBar(activity);
        addTranslucentViewWithColor(activity, color);
    }
    ...
    private static void addTranslucentViewWithColor(Activity activity, int color) {
        ViewGroup contentView = (ViewGroup) activity.findViewById(android.R.id.content);
        View fakeTranslucentView = contentView.findViewById(FAKE_TRANSLUCENT_VIEW_ID);
        if (fakeTranslucentView != null) {
            if (fakeTranslucentView.getVisibility() == View.GONE) {
            ...
            contentView.addView(createTranslucentStatusBarView(activity, 112));
        }
    }
