...
//        manager = ((WindowManager) context.getApplicationContext().getSystemService(Context.WINDOW_SERVICE));
    ...
    public static void setNotificationBarVisibility(Context ctx, final boolean isVisible) {
        String methodName;
        if (isVisible) {
     ...
     * @return {@code true}: yes<br>{@code false}: no
     */
    public static boolean isSupportNavBar(Context ctx) {
    ...
    private static void invokePanels(Context ctx, final String methodName) {
        try {
...
//        return Color.argb(Math.max(alpha, 1), red, green, blue);
        red = (int) (red * a + 0.5);
