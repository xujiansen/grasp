package com.rooten.frame.widget;
...
public class SegmentedGroup extends RadioGroup {
    ...
    private boolean mIsFinishInflate = false;
    ...
    public void updateBackground() {
        ...
        if (count > 1) {
            ...
            if (getOrientation() == LinearLayout.HORIZONTAL) {
                updateBackground(getChildAt(0), R.drawable.shape_checked_left, R.drawable.shape_unchecked_left);
            } else {
                updateBackground(getChildAt(0), R.drawable.shape_checked_top, R.drawable.shape_unchecked_top);
            }
            for (int i = 1; i < count - 1; i++) {
                // Check orientation for proper layout
                if (getOrientation() == LinearLayout.HORIZONTAL) {
                    updateBackground(getChildAt(i), R.drawable.shape_checked_middle, R.drawable.shape_unchecked_middle);
                } else {
                    // Middle radiobutton when checked is the same as horizontal.
                    updateBackground(getChildAt(i), R.drawable.shape_checked_middle, R.drawable.shape_unchecked_middle_vertical);
                }
            ...
            // Check orientation for proper layout
            if (getOrientation() == LinearLayout.HORIZONTAL) {
                updateBackground(getChildAt(count - 1), R.drawable.shape_checked_right, R.drawable.shape_unchecked_right);
            } else {
                updateBackground(getChildAt(count - 1), R.drawable.shape_checked_bottom, R.drawable.shape_unchecked_bottom);
            }
        } else if (count == 1) {
            updateBackground(getChildAt(0), R.drawable.shape_checked_default, R.drawable.shape_unchecked_default);
        }
    }
    ...
    private void updateBackground(View view, int checked, int unchecked) {
        ...
        ColorStateList colorStateList = new ColorStateList(new int[][]{
                ...
                {android.R.attr.state_checked, -android.R.attr.state_enabled},
                {-android.R.attr.state_checked, -android.R.attr.state_enabled}},
                new int[]{mTintColor, mCheckedTextColor, Color.parseColor("#eaeaea"), Color.parseColor("#FFDBDBDB")});
